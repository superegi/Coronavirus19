...
class Utilitarios:
    def __init__(self):
        ...
        print("Inicio módulo de funciones auxiliares")
        print("Las funciones son:")
        print("TS: contador de segundos del programa corriendo")
        print("guardador: funcion exportadora de BD para .csv .xlsx .pkl")
    ...
    def guardador(self,BD,nombre,tipo):
        if tipo=='csv':
            return BD.to_csv(nombre+'.'+tipo,
                      encoding='utf-8-sig',
                      index=False
                      )
        if tipo=='xlsx':
            return BD.to_excel(nombre+'.'+tipo,
                        index=False
                        )
        if tipo=='pkl':
            return BD.to_pickle(nombre+'.'+tipo)
